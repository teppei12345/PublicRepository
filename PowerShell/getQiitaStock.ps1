...
$qiitaApiUrl = 'https://qiita.com/api/v2/users/' + $userName + '/stocks'
# アクセストークン
...
# Qiitaのストック記事を登録するサイトのサイトIDを指定
$qiitaSiteId = '10064958'
# メール送信APIを実行する際のダミーレコードIDを指定
$sendMailRecordId = '10064960'
# APIバージョンを指定
$apiVersion = 1.1
# 宛先メールアドレスを指定
$toMailAddress = 'urata@implem.co.jp'
# 通知メールの件名を指定
$mailTitle = 'Qiitaのストック記事を登録しました。'
# 通知メールの本文を指定
$mailBodyText = 'Qiitaのストック記事をプリザンターに登録しました。該当テーブルは下記になります。ご確認をよろしくお願いいたします。'
...
$createLogDirectoryPath = 'C:\Implem\個人フォルダ_その他諸々\batch\Qiita\getQiitaStock\logFile'
# 現在日時を取得
...
# 「Qiitaのストック記事を取得する処理」が開始したことをログファイルに出力
$comment = '[' + $(Get-Date -Uformat %Y%m%d_%H:%M:%S) + '] 【DEBUG】「Qiitaのストック記事を取得する処理」の開始'
Add-Content -Path $createLogFilePath -Value $comment
# Qiitaのストック記事内容を取得する処理が開始したことをログファイルに出力
$comment = '[' + $(Get-Date -Uformat %Y%m%d_%H:%M:%S) + '] 【DEBUG】Qiitaのストック記事内容を取得する処理が開始しました。'
Add-Content -Path $createLogFilePath -Value $comment
# URLを設定する
$url = $qiitaApiUrl -f $userName
# URLをログファイルに出力
...
# Qiitaのストック記事情報が存在した場合の処理
if ($response) {
    # Qiitaのストック記事情報を読み込む反復処理が開始したことをログファイルに出力
    $comment = '[' + $(Get-Date -Uformat %Y%m%d_%H:%M:%S) + '] 【DEBUG】Qiitaのストック記事情報を読み込む反復処理が開始しました。'
    Add-Content -Path $createLogFilePath -Value $comment
    # カウンタを指定
    $count = 1
    # Qiitaのストック記事情報を1件ずつ読み込む
    ...
    $comment = '-----------------------------------------------------------------------------'
    Add-Content -Path $createLogFilePath -Value $comment
    # Qiitaのストック記事情報を読み込む反復処理が終了したことをログファイルに出力
    $comment = '[' + $(Get-Date -Uformat %Y%m%d_%H:%M:%S) + '] 【DEBUG】Qiitaのストック記事情報を読み込む反復処理が終了しました。'
    Add-Content -Path $createLogFilePath -Value $comment
}
